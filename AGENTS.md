...
- `internal/pgs/git` 覆盖完整：`loose_test`/`pack_test`/`refs_test`/`reach_test`/`browse_test`/`protocol_test`（基础读写、pack 编解码与真实 git pack 互验、ofs-delta、ref CAS/symref/packed-refs、SetHead 原子写、可达性 BFS、treeIsh 解析/tree 遍历/blob 读取/ForEachRefs、v0 状态机+sideband+空仓库回环）。`go test ./...` 通过。
