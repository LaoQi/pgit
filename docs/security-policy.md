...
> 最后更新：2026-09-23（TLS 相关已移出计划，见 §2.4 与 §5）。
...
| 传输加密 | 仅 `net.Listen`，无 TLS；HTTP 侧凭据可被中间人窃取 | 口令/内容泄露（**计划外**：由外层代理终止 TLS，见 §2.4） |
...
- **pgit 不实现 TLS**，也不引入 `tlsCert`/`tlsKey` 配置或 ACME。
- 需要 HTTPS 时在 pgit 前放反向代理（nginx/Caddy 等）终止 TLS 并转发到 pgit 的 HTTP 端口；
  或仅在可信内网（SSH 隧道、WireGuard 等）内使用。
- 该决策的理由：自研 TLS 证书配置与管理会引入配置面与状态管理复杂度，
  而反向代理在证书轮转、ACME、HSTS 等方面更成熟；pgit 保持单端口无状态、职责单一。
...
另注：pgit 自身不提供 HTTPS（见 §2.4），若原先依赖 pgit 直出 TLS，需改为「反向代理终止 TLS + 转发 HTTP」。

## 5. 明确不做（当前阶段）

- **不做 TLS / 证书配置 / ACME**（决策 C）：HTTPS 交由外层反向代理终止。
